...
public class AIController : MonoBehaviour
{
    ...
    public List<CurrentSelection> randomBestSelection = new List<CurrentSelection>();
    ...
    public void playMove()
    {
        ...
        GetMen1Piece();
        FindManWithBestMove();
        if (bestSelection.value == 1)
        {
            bestSelection = randomBestSelection[Random.Range(0, randomBestSelection.Count)];
        }

        GameManager.instance.MoveAI(Board.instance.boardSegments[bestSelection.AIPiecelocation.x, bestSelection.AIPiecelocation.y].segment.transform);
        GameManager.instance.MoveAI(Board.instance.boardSegments[bestSelection.highLightLoc.x, bestSelection.highLightLoc.y].segment.transform);
    }
    ...
    private void FindManWithBestMove()
    {
        foreach (var manLocation in MoveableMenLocationList)
        {
            ...
            GameManager.instance.MoveAI(Board.instance.boardSegments[manLocation.x, manLocation.y].segment.transform);
            ...
            foreach (var highLightLoc in highLightLocList)
            {
                ...
                if (globalValue == 1)
                {
                    ...
                    temp.AIPiecelocation = manLocation;
                    temp.highLightLoc = highLightLoc;
                    temp.value = globalValue;
                    randomBestSelection.Add(temp);
                }
            }
